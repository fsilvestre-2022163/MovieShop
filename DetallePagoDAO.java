...
public class DetallePagoDAO {
    ...
    public List listar(){
        ...
        try {
            ...
            while(rs.next()){
                ...
                dp.setIdPago(rs.getInt(4));
                dp.setIdCesta(rs.getInt(5));
            } 
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public int agregar (DetallePago dep){
        String sql = "nsert into DetallePago( cantidad, idProducto, idPago, idCesta)values(?,?,?,?)";
        try {
            con = cn.Conexion();
            ps = con.prepareStatement(sql);
            ps.setInt(1, dep.getCantidad());
            ps.setInt(2, dep.getIdProducto());
            ps.setInt(3, dep.getIdPago());
            ps.setInt(4, dep.getIdCesta());
            ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
